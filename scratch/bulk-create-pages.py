...
base = r"d:\DRIVE (Ai) Agents\00 Projects\Meer Foundation Website"

def write_page(relative_path, title, meta_desc, canonical, depth, h1, section_tag, body_content):
    asset_path = "../" * depth
    comp_script = f"{asset_path}assets/js/components-sub.js?v=4"
    core_script = f"{asset_path}assets/js/core.js?v=4"
    css = f"{asset_path}assets/css/style.css?v=4"
...
<script src="{comp_script}"></script>
<script src="{core_script}"></script>
</body>
</html>"""
    
    full_path = os.path.join(base, relative_path.replace("\\", os.sep).replace("/", os.sep))
...
all_pages = pages + legal_pages

for page in all_pages:
    write_page(page["path"], page["title"], page["desc"], page["canon"], page["depth"], page["h1"], page["tag"], page["body"])

print("Successfully created all pages.")
